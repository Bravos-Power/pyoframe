...
def test_retrieving_duals(solver, sense):
    ...
    assert m.A.solution == approx(45 * flip, **get_tol(solver))
    assert m.B.solution == approx(10 * flip, **get_tol(solver))
    assert m.objective.value == approx(29 * flip, **get_tol(solver))
    tight, slack = (
        (m.max_AB, m.min_AB) if sense == pf.ObjSense.MAX else (m.min_AB, m.max_AB)
    )

    assert tight.dual == approx(0.1, **get_tol(solver))
    assert slack.dual == approx(0, **get_tol(solver))

    if solver.name == "gurobi":
        assert tight.attr.slack == approx(0, **get_tol(solver))
        assert slack.attr.slack == approx(-200 * flip, **get_tol(solver))
